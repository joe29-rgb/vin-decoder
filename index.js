...
async function decodeVIN(vin) {
  try {
      ...
      return {
          'contact.year_of_trade': results.find(item => item.Variable === 'Model Year')?.Value || null,
          'contact.make_of_trade': results.find(item => item.Variable === 'Make')?.Value || null,
          'contact.model_of_trade': results.find(item => item.Variable === 'Model')?.Value || null,
          'contact.trade_in_trim': results.find(item => item.Variable === 'Trim')?.Value || null
      };
  } catch (error) {
